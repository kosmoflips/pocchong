...
<div class="mgarchive-overlay">[ <?php echo $entry['vol'] ?> ]<br /><?php echo $entry['title'] ?><br /><?php echo clock27($entry['epoch'],5) ?></div>
</div>
<?php
}
...
function process_data_mg_index ($pobj=null,$page=0) {
	...
	$stat=sprintf ('SELECT %s.id "id",vol,title,epoch,img_url FROM %s join %s on %s.rep_id = %s.id ORDER BY id DESC LIMIT ?,?', POC_DB['MYGIRLS']['table'], POC_DB['MYGIRLS']['table'], POC_DB['MYGIRLS']['table_pcs'], POC_DB['MYGIRLS']['table'], POC_DB['MYGIRLS']['table_pcs']);
	$list=$k->getAll($stat, array($offset,POC_DB['MYGIRLS']['max_gallery']));
	$page_title=sprintf ('%s::%s-%s',POC_DB['MYGIRLS']['title2'],$list[(count($list)-1)]['vol'],$list[0]['vol']);
