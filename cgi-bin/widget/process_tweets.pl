...
# forgot how to control number of tweets retrieved. but enough for "recent update". will set up number in php

#be careful on file location
my ($rawfile, $outfile)=@ARGV;
if (!$rawfile or !-e $rawfile or -z $rawfile or !$outfile) {
...
while ($chunk=~/<li\s+class=\\"timeline-TweetList-tweet customisable-border.+?Tumblr<\\\/a>/g) {
	...
	if ($entry=~m{<h2 class=\\"TwitterCard-title.+?>(.+?)<\\/h2>}) { $tweet->{note}=$1; } #forget what's this
	if ($entry=~m{<p class=\\"tcu-resetMargin.+?>(.+?)<\\/p>}) { $tweet->{note2}=$1; } #forget what's this
...
if (!$outfile) {
	$outfile=$rawfile.'_parsed.json';
}
# open (my $fh2,">:encoding(utf8)", $outfile);
open (my $fh2,">", $outfile);
print $fh2 encode_json $all;

return 1;

# old code, write display html directly to file
=pod
