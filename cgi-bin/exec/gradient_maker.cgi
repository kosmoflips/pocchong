...
<html lang="en">
<head>
<title>Gradient Maker ～彩～ | Pocchong.de</title>
<style>
	body {
...
<div>acceptable formats: HEX, HEX 3-letter, RGB, separate by whitespaces<br />
<a href="https://gist.github.com/kosmoflips/3faf44f6a2ac4e5efc5a#file-exec_gradient_maker-pl">offline version</a></div>

<form action="/e/gradient_maker.cgi" method="post">
<div style="text-align:left;width:800px;border: 1px solid #aaa;margin:10px auto">
<ul>';
printf '	<li>starting colour <input type="text" maxlength="15" size="6" value="%s" name="zero"></li>', ($zero?print_colour_code($zero):'#FFFFFF');
printf '	<li>x division: <input type="text" size="80" value="%s" name="tbx" /></li>', ($p->{tbx}||'C98EFF 467BDF');
printf '	<li>y division: <input type="text" size="80" value="%s" name="tby" /></li>', ($p->{tby}||'E6FF3A faa');
printf '	<li>mix step: <input type="number" min="0" max="255" size="2" value="%s" name="lv"> theoretically 0~255, but big numbers will eat up the RAM and also the screen...</li>', (($opt->{lv} and $opt->{lv}>2)?($opt->{lv}-1):5);
printf '	<li>alpha (0~1): <input type="number" step="0.05" min="0" max="1" size="3" value="%s" name="alfa"> | background: <input type="text" maxlength="15" size="6" value="%s" name="bg"></li>', ($opt->{alfa}||1), ($opt->{bg}?print_colour_code($opt->{bg}):'#FFFFFF');
